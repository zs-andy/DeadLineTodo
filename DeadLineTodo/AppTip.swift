...
struct AddContentTip: Tip{
    ...
    var rules: [Rule]{
        #Rule(Self.presentEvent) { event in
...
struct SetDeadlineTip: Tip{
    ...
    var rules: [Rule]{
        #Rule(Self.setStartTimeEvent) { event in
            event.donations.count >= 1
        }
    }
}
...
struct SetDurationTip: Tip{
    ...
    var rules: [Rule]{
        #Rule(Self.setDeadlineEvent) { event in
            event.donations.count >= 1
        }
    }
}
...
struct EmergencyViewTip: Tip{
    ...
    var message: Text? {
        Text("计划中已开始的任务将会显示在这里，尽快完成哦！")
    }
    var rules: [Rule]{
        #Rule(Self.emergencyViewEvent) {
            $0.donations.count == 1
        }
    }
}
