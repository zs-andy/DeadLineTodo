...
struct SearchTodoView: View {
    ...
    func done(todo: TodoData, doneDate: Date){
        if todo.addDate.timeIntervalSince1970 <= Date().timeIntervalSince1970{
            ...
            if todo.doing {
                todo.doing = false
                notificationService.cancelPendingNotification(withIdentifier: todo.id.uuidString + "4")
            ...
            for i in 1...4{
                notificationService.cancelPendingNotification(withIdentifier: todo.id.uuidString + "\(i)")
            }
            ...
            if todo.repeatTime != 0 {
                ...
                notificationService.sendOneToThreeNotifications(todo: repeatTodo)
                reminderService.copyEventToReminders(title: repeatTodo.content, priority: repeatTodo.priority, dueDate: repeatTodo.endDate, remindDate: repeatTodo.emergencyDate, edittodo: repeatTodo)
            ...
            ScrollView{
                LazyVStack{
                    ForEach(tododata.indices, id: \.self){index in
                        if filterTodos(todo: tododata[index]) {
                            ZStack{
                                HStack(){
                                    Spacer()
                                        ...
                                        HStack{
                                            ...
                                            Button(action: {
                                                if tododata.indices.contains(index) && allowToTap{
                                                    ...
                                                    for i in 1...4 {
                                                        notificationService.cancelPendingNotification(withIdentifier: tododata[index].id.uuidString + "\(i)")
                                                    }
                                                        ...
                                                        withAnimation(.default){
                                                            if tododata[index].addDate.timeIntervalSince1970 <= Date().timeIntervalSince1970{
                                                                if tododata[index].doing {
                                                                    ...
                                                                    notificationService.sendNotification1(todo: tododata[index])
                                                                }else{
                                                                    tododata[index].doing = true
                                                                    tododata[index].startDoingDate = Date()
                                                                    tododata[index].actualFinishTime = 0
                                                                    notificationService.sendNotification4(todo: tododata[index])
                                                                    notificationService.cancelPendingNotification(withIdentifier: tododata[index].id.uuidString + "1")
                                                                }
                                                            }else{
                                                                isAddDateAlertPresent = true
                                                            }
                                    ...
                                    Button(action: {
                                        ...
                                        ZStack(alignment: .topLeading){
                                            if getSize(todo: tododata[index], width: rowWidth ?? 0) != rowWidth{
                                                ...
                                                //.offset(x: getOffset(todo: tododata[index], width: rowWidth ?? 0))
//                                            if tododata[index].doing{
//                                                Rectangle()
